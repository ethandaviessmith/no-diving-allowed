...
func mood_update(curr_action, state, is_swimming, in_puddle) -> void:
	...
	match state:
		...
		Swimmer.State.IN_LINE, Swimmer.State.WANDERING:
			if randf() < 0.2 - pf_child_leisure:
				change_happy(-amt_low)
		Swimmer.State.ACT, Swimmer.State.APPROACH:
			if randf() < 0.2 - pf_child_leisure:
				change_happy(amt_low)
			if randf() < 0.2:
				pass
	if randf() < 0.5:
		if in_puddle and not Util.is_state(state, Swimmer.State.IN_LINE):
			change_safety(-0.2)
			SFX.play("puddle")
	else:
		change_safety(0.05)
	...
	if randf() > safety:
		...
		if not is_swimming and Util.is_state(state, Swimmer.State.IN_LINE):
			if randf() > clean + pf_child_athlete and randf() > 0.1 + pf_child_athlete:
				swimmer.throw_trash()
		elif not is_swimming and Util.is_state(state, [Swimmer.State.IDLE, Swimmer.State.APPROACH, Swimmer.State.WANDERING]):
