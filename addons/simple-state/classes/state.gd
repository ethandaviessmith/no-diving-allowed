...
## Returns the deepest active substate, or self if there are none.
func get_deep_active_state() -> State:
	var node: State = self
	while node.get_active_substate() != null:
		node = node.get_active_substate()
	return node
